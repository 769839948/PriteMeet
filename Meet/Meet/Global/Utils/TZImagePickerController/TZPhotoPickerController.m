...
- (void)viewDidLoad {
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"预览" style:UIBarButtonItemStylePlain target:self action:@selector(previewButtonClick)];
    [self.navigationItem.rightBarButtonItem setTitleTextAttributes:@{NSFontAttributeName:NavigationBarRightItemFont} forState:UIControlStateNormal];
...
- (void)configCollectionView {
    ...
    if (iOS7Later) top += 20;
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(margin, top, self.view.tz_width - 2 * margin, self.view.tz_height - 64 - top) collectionViewLayout:layout];
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        if (isSelected) {
            ...
            if (tzImagePickerVc.selectedModels.count < tzImagePickerVc.maxImagesCount) {
                ...
                NSString *title = [NSString stringWithFormat:@"最多选择 %ld 张照片", (long)tzImagePickerVc.maxImagesCount];
