...
{
    if (_window == nil) {
        _window = [[UIWindow alloc] initWithFrame:CGRectMake(0, ScreenHeight, ScreenWidth, 290.f)];
    }
    return _window;
}
...
- (UIButton*)closeButton
{
    if (_closeButton == nil) {
        ...
        _closeButton.frame = CGRectMake(ScreenWidth - 40.f, kDefaultTop, 30.f, 30.f);
...
- (void)didSelectMessageButton
{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        self.window.top = ScreenHeight - self.window.height;
    }];
}
...
- (void)keyboardWillChangeFrame:(NSNotification *)notification
{
    ...
    if ([self.window isKeyWindow]) {
        if (y == ScreenHeight) {
            [UIView animateWithDuration:0.3 animations:^{
                self.window.top = ScreenHeight - 290.f;
                self.window.height = 290.f;
            }];
        } else  {
            [UIView animateWithDuration:0.3 animations:^{
                self.window.top = 0;
                self.window.height = ScreenHeight;
            }];
        }
    }
}
